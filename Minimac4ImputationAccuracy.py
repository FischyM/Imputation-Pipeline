...
def setUp(argv, targetFileName, refFileName, cpus):
    ...
    parser = argparse.ArgumentParser(description='Minimac4ImputationAccuracy.py \
        -t <target file> -r <reference file> -cpu <default=4>\n')
    parser.add_argument(
        '-t', '--tar', required=True, help='target GWAS dataset as vcf.gz')
    parser.add_argument(
...
def checkPrograms():
    ...
    if not os.path.isdir('map'):
        print('directory "map" not found, be sure to download from our \
            GitHub and unzip plink.GRCh23.map.zip')
        sys.exit(1)

    # set all programs used to the env PATH variable, except for beagle
    if not os.path.isfile('beagle.27Apr20.b81.jar'):
        print('Cannot find beagle.27Apr20.b81.jar. Please download it to \
...
def separateFile(file, type):
    ...
    if file[-2:] == 'gz':
        ...
        if 'BGZF-compressed' not in stdOut[0].decode("utf-8"):
            ...
            print('In order to efficiently divide this file, it needs to be \
                decompressed and then recompressed with bgzip')
        ...
        filegz = file  # to stay consistent between if-else statements
    ...
    for i in range(1, 23):
        ...
        runCommandLine(
            ['tabix ../../' + filegz + ' -h ' + chr + ' > chr' + chr + '.vcf'])
    os.chdir(startDir)


def removeDuplicates(chr):
    # remove duplicate entries. Needed for pre-phasing
...
def impute(chr, cpu):
    # Used Minimac4 for imputation and the just created m3vcf reference file
...
def setUpCompare(infoFile):
    # Convert .info file from Minimac4 into a data frame
...
def printRsqEmpRsq(df, countGeno, dict_out, maf):
    # Finds the genotyped statistics used for accuracy calculation
...
def rsqCutoffs(mafList, dict_out, type):
    # Determine rsq ratios at different cutoff limits for MAF >= 5%
    ...
    for val in cutoffs:
        ...
        dict_out[type]['MAF > 5%, Rsq : snps ratio'][round(val, 2)] = \
            round(newSnps/float(totalSNPs), 4)


def printRsq(df, countTotal, dict_out, maf):
    # Determines rsq accuracy calculation for all snps
...
def compare(infoFile, chr):
    # Uses the above functions for a full analysis of the imputation accuracy
    dict_out = {
        'genotyped-snps': {
            'MAF <= 0.05%': {},
            'MAF 0.05-5%': {},
            'MAF >= 5%': {},
            'correlation': None,
            'MAF > 5%, Rsq : snps ratio': OrderedDict()},
        'all-snps': {
            'MAF <= 0.05%': {},
            'MAF 0.05-5%': {},
            'MAF >= 5%': {},
    ...
    mafGreater_5_EmprNeg = mafGreater_5.loc[(mafGreater_5['EmpR'] <= 0.0)]
    mafGreater_5_EmprPos = mafGreater_5.loc[(mafGreater_5['EmpR'] > 0.0)]
    mafGreater_5_EmprNeg.plot.scatter(
        x='Rsq', y='EmpRsq', c='red', legend=True)
    mafGreater_5_EmprPos.plot.scatter(
        x='Rsq', y='EmpRsq', c='green', legend=True)
    plt.savefig(
        'output/chr' + chr + '/scatter_Genotypes.EmpRsq.Rsq_MAF_GT_5.png')
...
def writeTSV(dictIn):
    # Writes the dictionary to a tab-delimited csv file
...
def main():
    ...
    targetFileName, refFileName, cpus =\
        setUp(sys.argv[1:], targetFileName, refFileName, cpus)
    checkPrograms()
    ...
    for i in range(1, 23):
        ...
        infoFile = \
            'output/chr' + chromosome + '/imputed_chr' + chromosome + '.info'
